...
with known locations. A 'Map' contains information for only one storey.
...
class Map:
    ...
    def getNode(self, nodeId):
        if nodeId in self.nodesDict:
            ...
            print('Error when calling Map.getNode(): Node #' + nodeId + ' does not exist.')
            return None
    
    def addEdge(self, srcNodeId, destNodeId, weight = 0):
        if srcNodeId not in self.nodesDict:
            print('Error when calling Map.addEdge(self, ' + srcNodeId + ', ' + destNodeId + ', ' + weight + '): ' \
                  '\'srcNodeId\' #' + srcNodeId + ' does not exist.')
        elif destNodeId not in self.nodesDict:
            print('Error when calling Map.addEdge(self, ' + srcNodeId + ', ' + destNodeId + ', ' + weight + '): ' \
                  '\'destNodeId\' #' + destNodeId + ' does not exist.')
        else:
            self.nodesDict[srcNodeId].addNeighbor(self.nodesDict[destNodeId], weight)
            self.nodesDict[destNodeId].addNeighbor(self.nodesDict[srcNodeId], weight)
...
class Node:
    ...
    def addNeighbor(self, neigNodeIdexId, weight = 0):
        self.adjacentNodes[neigNodeIdexId] = weight
